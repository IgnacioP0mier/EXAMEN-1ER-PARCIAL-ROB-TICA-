...
class FKClientB(Node):

    def __init__(self):
        super().__init__('fk_client_b')
        self.joints = None

        self.sub = self.create_subscription(
            JointState, 'joint_states', self.joint_callback, 10)

        self.client = self.create_client(GetPosition, 'get_position_b')
        while not self.client.wait_for_service(timeout_sec=1.0):
            self.get_logger().warn('Esperando fk_server_b...')
...
def main(args=None):
    ...
    if result:
        node.get_logger().info(
            f'Robot B → x={result.x:.4f}, y={result.y:.4f}'
        )
    else:
